...
def get_dataframe_AutoWWER_CIC_BMS():
    df = data_loader.get_dataframe("CIC_BMS")
    # 列字段名“Rpt Key”下的数据值的起始字母为CTR
    #列字段名“Billing Status”下的数据值等于“successful”.
    filtered_df = df[df['Rpt Key'].str.startswith('CTR') & (df['Billing Status'].str.lower() == 'successful')]
    return filtered_df


def get_dataframe_AutoWWER_GCG_BMS():
...
def get_dataframe_AutoWWER_CIC_CATS():
    df = data_loader.get_dataframe("CIC_CATS_WWER")
    # CATS Report无筛选条件， 把CIC和GCG下CATS Report的WWER sheet页面的数据全部抓取出来
    filtered_df = df
    return filtered_df


def get_dataframe_AutoWWER_GCG_CATS():
    df = data_loader.get_dataframe("GCG_CATS_WWER")
    # CATS Report无筛选条件， 把CIC和GCG下CATS Report的WWER sheet页面的数据全部抓取出来
    filtered_df = df
    return filtered_df

def write_AutoWWER_data_to_excel(time_range_from_billing_type_search, months_filter_for_BMS_CATS):
    ...
    try:
        # 获取数据
        df_CIC_BMS = get_dataframe_AutoWWER_CIC_BMS()
        df_GCG_BMS = get_dataframe_AutoWWER_GCG_BMS()
        df_CIC_CATS = get_dataframe_AutoWWER_CIC_CATS()
        df_GCG_CATS = get_dataframe_AutoWWER_GCG_CATS()
        ...
        if combined_df_CIC_BMS.empty and combined_df_GCG_BMS.empty and combined_df_CIC_CATS.empty and combined_df_GCG_CATS.empty:
            result_list.append("NG")
            print("当前输入的月份查找的AutoWWER_Report没有数据")
        else:
            # 设置文件路径，添加时间戳防止覆盖
            # timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            file_path_AutoWWER = f'report/AutoWWER_Report_{time_range_from_billing_type_search}.xlsx'
                # 写入Excel文件
            with pd.ExcelWriter(file_path_AutoWWER) as writer:
                ...
                if not combined_df_CIC_CATS.empty:
                    combined_df_CIC_CATS.to_excel(writer, sheet_name='CIC_WWER_CATS', index=False)
                if not combined_df_GCG_CATS.empty:
                    combined_df_GCG_CATS.to_excel(writer, sheet_name='GCG_WWER_CATS', index=False)

            print("查出的数据生成的AutoWWER_Report已保存在本地")
            result_list.append("OK")
            file_path_AutoWWER = [file_path_AutoWWER]
    except Exception as e:
        print(f"程序出错: {e}")

    return file_path_AutoWWER, result_list


if __name__ == "__main__":
    # df1 = get_dataframe_AutoWWER_CIC_BMS()
    # df2 = get_dataframe_AutoWWER_GCG_BMS()
    months_filter_for_BMS_CATS = get_month_range('202401-202403')
    write_AutoWWER_data_to_excel(1, months_filter_for_BMS_CATS)
